...
public interface PassportRepository extends JpaRepository<Passport, Long> {
    ...
    List<Passport> findByPerson_FirstNameOrPerson_LastNameOrPerson_BirthDateAllIgnoreCase(@Nullable String firstName, @Nullable String lastName, @Nullable String birthDate);

    @Query("select p from Passport p where year(p.person.birthDate) = ?1")
    List<Passport> findByPerson_BirthDate(String birthDate);
}